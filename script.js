...
function speichereSchichtUebergaben() {
    ...
    let datum = document.getElementById('date-display').value;
    ...
    // Prüfe das Format des Datums und konvertiere es bei Bedarf in ISO 8601 (YYYY-MM-DD)
    if (datum.includes("/")) {
        const [day, month, year] = datum.split("/");
        datum = `${year}-${month}-${day}`; // Umwandeln in YYYY-MM-DD
    }

    // Objekt erstellen, das an die Netlify-Funktion gesendet wird
    ...
    console.log("Sende Schichtübergabe:", schichtUebergabe);

    // Daten an die Serverless-Funktion senden
...
// Event Listener hinzufügen, um bei Änderung in den Auswahlfeldern zu speichern
